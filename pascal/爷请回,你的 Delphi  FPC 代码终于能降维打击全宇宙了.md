...
**简单说：你的 Pascal 代码调用 `API_Call`，zAPI 核心库转手就找到目标语言的服务，把结果快递回来——整个过程比你点外卖还快，你甚至感觉不到中间隔了十几层。**
...
| 特性 | 有多牛？ | v2.0 升级 |
|------|---------|----------|
| 🌍 **12+ 语言绑定** | Pascal（Delphi/FPC）、Node.js、PHP、Web.js、Python、C++、Go、Rust、Java、C#、VB.Net、C……全家桶都给你配齐了，随便挑 | ✅ PHP、Node.js 新增 |
...
| 🔧 **动态注销 API** | 运行时移除 API，触发网络广播（约 3 秒传播），适合热卸载插件 | ✅ **v2.0 新增** |
| ⚙️ **运行时配置** | 动态调整认证密码、等待连接、IPC 线程池等参数，无需重启 | ✅ **v2.0 新增** |
...
**看懂了吗？你的老 Delphi 系统一夜之间变成了“全能翻译官”，跟谁都能唠嗑。**
...
**你的 Pascal 代码不再是“历史包袱”，而是“企业核心资产”。**

---

## 🐛 调试小贴士

当服务或客户端出现问题时，首先**检查控制台输出** —— 库默认会将详细的运行日志（包括连接状态、注册信息、错误原因）打印到标准输出（`ConsoleOutput=True`）。你可以在 `<可执行文件名>.api-tool.ini` 配置文件中调整日志行为（例如关闭控制台输出或调整详细程度）。
...
| `no found app("XXX") api("YYY")` | 应用名或 API 名写错了 | 检查大小写，必须一模一样 |
| `timeout` | 超时 | 增加超时值，看看服务还在不在 |
...
A: **动态注销 API（`API_UnReg`）** —— 运行时移除 API，适合热卸载插件、临时维护、权限动态调整，约 3 秒广播传播。**运行时配置（`API_SetOption`）** —— 动态调整认证密码、等待连接行为、IPC 线程池等参数，无需重启应用。**PHP 和 Node.js 支持** —— 通过 ZAPI Bridge 实现双向调用，零原生依赖。
...
| 函数 | 干啥的？ | 新增 |
|------|---------|------|
| `API_Create_DataHnd` | 创建数据包裹 | |
| `API_Free_DataHnd` | 销毁包裹 | |
| `API_WriteBuffer` | 往包裹里塞数据 | |
| `API_ReadBuffer` | 从包裹里取数据 | |
| `API_GetPos` / `API_SetPos` | 查看/设置包裹里的“读指针”位置 | |
| `API_GetSize` / `API_SetSize` | 查看/调整包裹大小 | |
| `API_Create_APPHnd` | 注册你的“服务公司” | |
| `API_Free_APPHnd` | 注销公司 | |
| `API_Reg_Call` | 注册一个“电话客服” | |
| `API_Reg_Notify` | 注册一个“短信客服” | |
| `API_UnReg` | **“吊销执照”（v2.0）—— 运行时注销 API，热卸载** | ✅ |
| `API_SetOption` | **“调参数”（v2.0）—— 运行时配置，不用重启** | ✅ |
| `API_Local_APP_Call` | 本地测试（不联网） | |
| `API_Reset_Prepare` | 重置网络配置 | |
| `API_Prepare_Service` | 启动一个服务监听 | |
| `API_Prepare_Client` | 连到别人家的服务 | |
| `API_Prepare_Done` | 正式开张营业 | |
| `API_Exit_MainThread` | 打烊前通知 | |
| `API_Call` | 打电话（远程调用） | |
| `API_Notify` | 发短信（单向通知） | |
| `API_shutdown` | 彻底关门，释放资源 | |
