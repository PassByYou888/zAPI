...
> **v2.0 新特性：** 现在不仅 C++/Python/Go 能调，**PHP 和 Node.js** 也能通过 ZAPI Bridge 调用你的 Pascal 服务，**零原生依赖**。同时支持 **动态注销 API**（`API_UnReg`）和 **运行时配置**（`API_SetOption`），让你的服务治理能力再上一个台阶。
...
**结论：** zAPI 不是让 Pascal 变得“年轻”，而是让它直接“封神”，坐在那里等着别人来朝拜，或者它去巡视别人的领地。**v2.0 让 PHP 和 Node.js 也成了朝拜队伍的一员。**

---

## ⚙️ 核心作战单元（老司机 5 分钟扫盲）

看代码之前，先记住这四个比美女还让你惦记的概念：
...
| **PHP (v2.0 新增)** | ✅ | Web 后端的老大哥，现在也要来调你的老代码。 | ✅ 通过 Bridge 双向调用 |
| **Node.js (v2.0 新增)** | ✅ | 前端小年轻也得通过网关来调你的老古董。 | ✅ 通过 Bridge 双向调用 |
| **Web.js (浏览器)** | ✅ | 浏览器直接调，前端也要用你的 Pascal 算力。 | — |

**结论：** 你的老代码一夜之间变成了“全语言翻译官”，从被时代遗忘的角落，直接空降到技术生态的 C 位。**v2.0 让 PHP 和 Node.js 也加入了这个阵营。**
...
**Q5：v2.0 新增了啥？我该关注哪些？**
A：**① 动态注销 API（`API_UnReg`）** —— 运行时移除 API，支持热卸载插件、临时维护、权限动态调整，约 3 秒广播传播。**② 运行时配置（`API_SetOption`）** —— 动态调整认证密码、等待连接行为、IPC 线程池等参数，无需重启应用。**③ PHP 和 Node.js 支持** —— 通过 ZAPI Bridge 实现双向调用，零原生依赖，Web 生态全面接入。

**Q6：动态注销后，正在进行的调用会怎样？**（v2.0 新增）
