        ...
        begin
          ...
          if k0 mod 1000 = 0 then // 每执行1000次刷新一次界面，否则刷新不过来
            TThread.Synchronize(nil,
              ...
              begin
                Text5.text := '总次数' + (k0).ToString + '---成功' + (k1).ToString + '---失败' + (k2).ToString + Format('耗时:%f秒', [(TThread.GetTickCount - n) / 1000]) + Format('总耗时:%f秒',
                  [(TThread.GetTickCount - n1) / 1000]) + Format('并发:%d次/秒', [round(k1 / ((TThread.GetTickCount - n1) / 1000))]);
              end);
