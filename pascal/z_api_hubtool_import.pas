...
{ ------------------------------------------------------------------------------
  ...
  ── TCritical ──
  对 TCriticalSection 的简单封装，提供 Lock / UnLock 方法。
  用于保护全局资源，确保线程安全。

  ── TOrderStruct<T_> ──
  非线程安全的 FIFO 链表（单向链表），提供 Push、Next、Clear 操作。
  用于构建消息队列或对象池。内部不包含锁，由调用者保证同步。
  ...
  原理：
    - 每个线程可拥有一个 Soft_Synchronize_Tool 实例。
    - 调用 Synchronize 将回调（TOnSynchronize_P_NP）加入队列，
      并设置一个 Boolean 信号为 True。
    - 目标线程（通常是主线程）定期调用 Check_Synchronize，
      从队列取出回调执行，并将信号置为 False。
    - 调用 Synchronize 的线程会忙等待该信号直到 False，
      从而实现同步。

  注意：Soft_Synchronize_Tool 内部使用 TCritical 保护队列，
  因此是线程安全的。
  ------------------------------------------------------------------------------ }
type
  // 迷你临界区封装
  ...
  public
    ...
    procedure SwapInstance(source: TOrderStruct<T_>); // 仅交换数据，保留 OnFree
    procedure Clear;
...
{$ELSE FPC}
  Soft_Sync.Synchronize(procedure
    begin
      ev(Input, Output);
    end);
{$ENDIF FPC}
...
begin
  ...
  if not IsLibrary then
    begin
      if IsConsole then
        API_SetOption2('ConsoleOutput', 'True')
      else API_SetOption2('ConsoleOutput', 'False');
    end;
end;
