...
访问 https://www.lazarus-ide.org 下载安装包，推荐最新稳定版（如 Lazarus 2.2.6+）。安装路径建议为 `C:\lazarus`。

### 2. ⚡ 配置系统 PATH（最方便的方式）

**将 Lazarus 安装目录（例如 `C:\lazarus`）添加到系统 `PATH` 环境变量中。** 这样您可以在任何目录下直接运行 `lazbuild`，无需每次指定完整路径。

- 打开 **系统属性** → **高级** → **环境变量**
- 在 **系统变量** 或 **用户变量** 中找到 `Path`，编辑并添加 Lazarus 目录（如 `C:\lazarus`）
- 确定保存，重启命令行窗口生效
...
如果看到版本信息，说明 PATH 配置成功。

> 💡 **不想修改系统 PATH？** 也可以在命令行中临时添加：
> ```cmd
...
**产物**：`z_api_hub32.dll` / `z_api_hub64.dll`（位于 `Src\` 目录）
